...
#endif
unsigned char rxd0[4]={myID,0x00,0x00,0x00}; //判断发过来的牌是否乱码（错误） 
unsigned char a=0x00;													//LED灯
unsigned char rxd[4];      										//接收数据
unsigned char send[4]={myID,0x00,0x00,0x00};   //重复发送（K2)
unsigned char send1[4]={myID,0x00,0x00,0x00};  //发出去的牌(K1)
unsigned char send2[4]={myID,0x00,0x00,0x00};  //现有手牌
